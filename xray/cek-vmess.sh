#!/bin/bash
# ==========================================
# CEK LOGIN VMESS USER (Refactored)
# ==========================================
...
# Mengambil log dari 10000 baris terakhir, cari yang accepted via VMess (ws/grpc/none)
# Pola log xray: [waktu] [ip:port] accepted tcp:... email: [username]
active_log=$(tail -n 10000 "$LOG" | grep -w "accepted" | grep -iE 'email:.*')

if [[ -z "$active_log" ]]; then
    echo -e "${RED}Belum ada user VMess yang online / terekam.${NC}"
else
    # Mengumpulkan semua user unik dari log
    users=$(echo "$active_log" | awk -F'email: ' '{print $2}' | awk '{print $1}' | sort -u)
    
    for user in $users; do
        # Cek apakah user tersebut benar-benar ada di database vmess.db kita
        if grep -qw "^${user}" /etc/xray/vmess.db 2>/dev/null; then
