#!/bin/bash
# ==========================================
# RENEW VMESS ACCOUNT (Refactored)
# ==========================================
...
if [[ ! -s "$DB" ]]; then
    echo -e "\n${RED}❌ Tidak ada user VMess yang terdaftar!${NC}"
    read -n 1 -s -r -p "Tekan apa saja untuk kembali..."
    m-vmess
    exit 0
fi

echo -e "\n${CYAN}📋 Daftar User VMess:${NC}\n"
...
if [[ $old_exp_ts -lt $today ]]; then
    ...
    # Jika belum expired, tambahkan dari sisa hari
    new_exp=$(date -d "$old_exp +$days days" +"%Y-%m-%d")
fi

# Update Database (Tidak perlu restart Xray karena Xray tidak mengurus masa aktif)
